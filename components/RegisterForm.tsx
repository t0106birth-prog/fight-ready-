...
export function RegisterForm({ gyms, presetGym }: { gyms: Gym[]; presetGym?: Gym | null }) {
  ...
  return (
    <form action={registerAction} className="card">
      ...
      <label className="fl">性別</label>
      <div className="seg wrap">
        <label><input type="radio" name="gender" value="male" />男性</label>
        <label><input type="radio" name="gender" value="female" />女性</label>
        <label><input type="radio" name="gender" value="other" />その他</label>
        <label><input type="radio" name="gender" value="no_answer" defaultChecked />回答しない</label>
      </div>
      ...
      {/* 所属ジム：QR/招待リンクなら固定表示。無ければチームコードを入力（お試しは選択も可） */}
      {presetGym ? (
        <>
      ...
      {/* 格闘技の種類 */}
      <label className="fl">
        格闘技の種類{userType === "pro" ? "（主競技を1つ＋副競技）" : "（任意・複数選べます）"}
      </label>
      {userType === "pro" && (
        <>
          <select name="primarySport" defaultValue="" required style={{ marginBottom: 8 }}>
            <option value="" disabled>主競技を選ぶ</option>
            {SPORTS.filter((s) => s.id !== "fitness").map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
          </select>
          <span className="info-note">↓ 副競技（あれば選ぶ）</span>
        </>
      )}
        ...
        <div className="card tight" style={{ marginTop: 16 }}>
          ...
          {promotion !== "one" && (
            <>
              ...
              <p className="info-note">アマチュア選手など、尿比重検査を使わない場合はチェックを外してください（水抜きの体重モニタリングは使えます）。ONE選択時は自動でONになります。</p>
            </>
          )}
          <div className="grid2">
            <div>
      ...
      <p className="info-note" style={{ marginTop: 10 }}>
        登録すると、健康に関するデータ（体重・体調・痛みなど）を記録・保存することに同意したものとします。
      </p>
    </form>
  );
}
